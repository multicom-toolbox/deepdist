...
if os.path.exists(f'{outdir}/X-{target}.txt') and os.path.exists(f'{outdir}/{target}.cov') and os.path.exists(f'{outdir}/{target}.plm') and os.path.exists(f'{outdir}/{target}.pre'):
    ...
    if os.path.exists(f'{outdir}/alignment/{target}.aln') and os.path.getsize(f'{outdir}/alignment/{target}.aln') > 0:
        print('Alignment already generated... skipped')
    else:
        ...
        if option == 'ALN':
            os.system(f'{db_tool_dir}/tools/DeepAlign1.0/hhjack_hhmsearch3.sh {fasta} {outdir}/alignment {db_tool_dir}/tools/ {db_tool_dir}/databases/ \
                {uniref90_dir}/uniref90 {hhsuitedb_dir}/{hhsuitedb_name} {metaclust50_dir}/metaclust_50')
        elif option == 'MSA':
            os.system(f'python {db_tool_dir}/tools/deepmsa/hhsuite2/scripts/build_MSA.py {fasta} -hhblitsdb={hhsuitedb_dir}/{hhsuitedb_name} \
                -jackhmmerdb={uniref90_dir}/uniref90 -hmmsearchdb={ebi_uniref100_dir}/myg_uniref100 -tmpdir={outdir}/alignment/tmp \
                    -outdir={outdir}/alignment -ncpu=8 -overwrite=0')
        else:
